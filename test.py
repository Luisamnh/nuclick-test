...
def main():
    ...
    modelSaveName = "%s/weights-%s.h5.h5" % (config.weights_path, modelBaseName)
    ...
    if config.application in ['Cell', 'Nucleus']:
        ...
        #cv2.imshow('Clicks', np.uint8(255 * np.sum(nucPoints, axis=(0, 3))))
        cv2.imshow('Segmentation', instanceMap_RGB.astype(np.uint8))
        cv2.waitKey(0)
